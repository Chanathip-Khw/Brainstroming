...
export default function DashboardPage() {
  ...
  useEffect(() => {
    if (status === 'authenticated' && session?.accessToken) {
      ...
      fetch(
        `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/api/workspaces`,
        {
          headers: {
  ...
  const fetchWorkspaceProjects = async (workspaceId: string) => {
    ...
    try {
      ...
      if (data.success && data.projects) {
        ...
        setTeams(prevTeams =>
          prevTeams.map(team =>
            team.id === workspaceId ? { ...team, boards } : team
          )
        );
      } else {
        console.error(
          `Error fetching projects: ${data.error || 'Unknown error'}`
        );
      }
    } catch (error) {
      console.error(
          ...
          members: [
            {
              ...
              avatar:
                data.workspace.owner.avatarUrl || '/api/placeholder/40/40',
            },
          ],
          boards: [],
        };
  ...
  const handleRenameBoard = async (boardId: string, newName: string) => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/api/projects/${boardId}`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
  ...
  const handleDuplicateBoard = async (boardId: string) => {
    const boardToDuplicate = teams
      .flatMap(team => team.boards)
      .find(board => board.id === boardId);
    if (!boardToDuplicate || !session?.accessToken) return;

    // Find which team this board belongs to
    const teamWithBoard = teams.find(team =>
      team.boards.some(board => board.id === boardId)
    );
    ...
    navigator.clipboard
      ...
      .catch(err => {
        ...
        try {
          ...
          console.error(
            'Fallback: Could not copy board link to clipboard:',
            err
          );
                ...
                members: [
                  {
                    ...
                    avatar:
                      workspace.owner.avatarUrl || '/api/placeholder/40/40',
                  },
                ],
      ...
      <main className='p-6'>
        <div className='mb-8'>
          ...
          <p className='text-gray-600'>
            Collaborate and brainstorm with your teams
          </p>
        </div>
