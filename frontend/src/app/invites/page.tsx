...
export default function InvitesPage() {
  ...
  const fetchInvites = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/api/workspaces/invites`, {
        headers: {
  ...
  const acceptInvite = async (token: string, inviteId: string) => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/api/workspaces/invites/accept`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        addNotification('success', data.message || 'Invite accepted successfully');
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="p-6 max-w-4xl mx-auto">
        <div className="mb-8">
          <h2 className="text-2xl font-bold text-gray-900 mb-2">Your Invitations</h2>
          <p className="text-gray-600">Accept or decline workspace invitations</p>
        </div>
        ...
        {invites.length === 0 ? (
          ...
          <div className="grid gap-4">
            {invites.map((invite) => (
              <div key={invite.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="text-xl font-semibold text-gray-900">{invite.workspace.name}</h3>
                    <p className="text-sm text-gray-500 mt-1">
                      Invited by {invite.workspace.owner.name} ({invite.workspace.owner.email})
                    </p>
