...
export default function SessionManager() {
  ...
  const fetchSessions = async () => {
    try {
      setLoading(true);
      const response = await fetchApi<{ sessions: Session[] }>('/api/auth/sessions');
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {sessions.map((session) => (
                <tr key={session.id} className={session.isCurrent ? "bg-blue-50" : ""}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{session.browser}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{session.ipAddress}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{formatDate(session.createdAt)}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{formatDate(session.expiresAt)}</td>
