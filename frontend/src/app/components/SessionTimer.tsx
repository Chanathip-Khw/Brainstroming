...
export const SessionTimer: React.FC<SessionTimerProps> = ({ onTimerComplete, currentTemplate, currentActivityIndex = 0, templateSessionId, isOpen: externalIsOpen, onOpenChange }) => {
  ...
  const [selectedActivity, setSelectedActivity] = useState<TimerActivity | null>(null);
      ...
      intervalRef.current = setInterval(() => {
        setTimeRemaining(prev => {
          if (prev <= 1) {
            ...
            if (selectedActivity && onTimerComplete && selectedActivity.id.startsWith('template-')) {
  ...
  const getProgressPercentage = () => {
    ...
    return ((selectedActivity.duration - timeRemaining) / selectedActivity.duration) * 100;
  };
        ...
        <div className="fixed bottom-20 right-4 z-40 bg-white border border-gray-200 rounded-lg shadow-lg p-3 min-w-[280px] max-w-[320px]">
          <div className="flex items-center justify-between mb-2">
            <h3 className="font-semibold text-sm">
              ...
              {currentTemplate && selectedActivity.id.startsWith('template-') && (
                <span className="text-xs text-purple-600 ml-2">
                  ({currentActivityIndex + 1}/{currentTemplate.activities.length})
                </span>
              )}
            </h3>
          ...
          <div className="text-xs text-gray-600 mb-2">
            ...
            {currentTemplate && selectedActivity.id.startsWith('template-') && currentTemplate.activities[currentActivityIndex] && (
              <div className="mt-2 p-2 bg-purple-50 rounded text-xs">
                <div className="font-medium text-purple-800 mb-1">Tips:</div>
                <ul className="text-purple-700 space-y-1">
                  {currentTemplate.activities[currentActivityIndex].tips.map((tip: string, index: number) => (
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full mx-4 max-h-[80vh] overflow-y-auto">
            <div className="p-6">
              ...
              <div className="space-y-3 mb-6">
                <h3 className="font-medium text-gray-700">Brainstorming Activities</h3>
                {PRESET_ACTIVITIES.map((activity) => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className={`px-2 py-1 rounded text-white text-xs ${activity.color}`}>
