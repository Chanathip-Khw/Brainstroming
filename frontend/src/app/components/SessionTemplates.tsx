...
const SESSION_TEMPLATES: SessionTemplate[] = [
  {
    ...
    description: 'Traditional ideation session for generating and organizing ideas',
    duration: 45,
    ...
    activities: [
      {
        ...
        tips: ['Clearly state the problem', 'Share background information', 'Set ground rules']
      },
      {
        name: 'Individual Ideation',
        duration: 10,
        description: 'Silent brainstorming - generate ideas individually',
        tips: ['No talking during this phase', 'Quantity over quality', 'Build on others ideas']
      },
      {
        name: 'Idea Sharing',
        duration: 15,
        description: 'Share and build on ideas together',
        tips: ['One idea at a time', 'No criticism or judgment', 'Ask clarifying questions']
      },
      {
        name: 'Affinity Mapping',
        duration: 10,
        description: 'Group related ideas together',
        tips: ['Look for themes and patterns', 'Create meaningful categories', 'Some ideas may stand alone']
      },
      {
        name: 'Dot Voting',
        duration: 5,
        description: 'Vote on the most promising ideas',
        tips: ['Give everyone equal votes', 'Consider feasibility and impact', 'No need to explain votes']
      }
    ]
  },
  {
    ...
    activities: [
      ...
      },
      {
        ...
        tips: ['Use different colored notes', 'One idea per note', 'Don\'t edit yourself']
      },
      {
        name: 'Idea Clustering',
        duration: 5,
        description: 'Quick grouping of similar ideas',
        tips: ['Move fast', 'Don\'t overthink', 'Some ideas can be in multiple groups']
      },
      {
        name: 'Quick Wins',
        duration: 5,
        description: 'Identify immediately actionable ideas',
        tips: ['Look for low-hanging fruit', 'Consider resource requirements', 'Mark for immediate action']
      }
    ]
  }
];
...
export const SessionTemplates: React.FC<SessionTemplatesProps> = ({ onStartTemplate, isOpen: externalIsOpen, onOpenChange }) => {
                ...
                {SESSION_TEMPLATES.map((template) => (
                  ...
                  >
                    ...
                    <div className="space-y-2">
                      <h4 className="text-xs font-medium text-gray-700">Activities:</h4>
                      <div className="space-y-1">
                        {template.activities.map((activity, index) => (
                          <div key={index} className="flex items-center justify-between text-xs">
                            <span className="text-gray-600">{activity.name}</span>
                            <span className="text-purple-600 font-medium">{activity.duration}min</span>
                          </div>
                        ))}
                      </div>
                    </div>
