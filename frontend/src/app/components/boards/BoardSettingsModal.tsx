...
export const BoardSettingsModal = ({ isOpen, onClose, boardName, onSaveSettings }: BoardSettingsModalProps) => {
  const [settings, setSettings] = useState<BoardSettings>({
    ...
    members: [
      { id: '1', name: 'John Doe', email: 'john@example.com', role: 'owner', avatar: '/api/placeholder/32/32' },
      { id: '2', name: 'Jane Smith', email: 'jane@example.com', role: 'editor', avatar: '/api/placeholder/32/32' },
      { id: '3', name: 'Mike Johnson', email: 'mike@example.com', role: 'viewer', avatar: '/api/placeholder/32/32' }
    ]
  });
      ...
      <div className="p-0 w-full max-w-2xl">
        ...
        <div className="p-6 max-h-96 overflow-y-auto">
            ...
            <div className="space-y-6">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setSettings({ ...settings, boardName: e.target.value })}
          ...
          {activeTab === 'permissions' && (
            <div className="space-y-6">
              <div>
                <h4 className="text-sm font-medium text-gray-700 mb-4">Board Permissions</h4>
                <div className="space-y-4">
                  <label className="flex items-center justify-between">
                    <div>
                      <div className="font-medium text-gray-900">Allow Comments</div>
                      <div className="text-sm text-gray-500">Members can add comments to elements</div>
                    </div>
                    <input
                      type="checkbox"
                  ...
                  <label className="flex items-center justify-between">
                    <div>
                      <div className="font-medium text-gray-900">Allow Voting</div>
                      <div className="text-sm text-gray-500">Members can vote on ideas</div>
                    </div>
                    <input
                      type="checkbox"
                      checked={settings.allowVoting}
                      onChange={(e) => setSettings({ ...settings, allowVoting: e.target.checked })}
                      className="w-4 h-4 text-indigo-600"
                    />
                  </label>
                  ...
                  <label className="flex items-center justify-between">
                    <div>
                      <div className="font-medium text-gray-900">Allow Export</div>
                      <div className="text-sm text-gray-500">Members can export board content</div>
                    </div>
                    <input
                      type="checkbox"
                  ...
                  {roles.map((role) => (
                    <div key={role.id} className="p-3 border border-gray-200 rounded-lg">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {role.id === 'editor' && <Shield className="w-4 h-4 text-blue-500" />}
                        {role.id === 'viewer' && <Eye className="w-4 h-4 text-gray-500" />}
