...
export const BoardOptionsModal = ({ 
  ...
  const options = [
    { icon: Edit, label: 'Rename Board', action: () => onEdit(board.id), color: 'text-gray-700' },
    { icon: Copy, label: 'Duplicate Board', action: () => onDuplicate(board.id), color: 'text-gray-700' },
    { icon: Share, label: 'Share Board', action: () => onShare(board.id), color: 'text-gray-700' },
    { icon: Trash2, label: 'Delete Board', action: () => onDelete(board.id), color: 'text-red-600' }
  ];
