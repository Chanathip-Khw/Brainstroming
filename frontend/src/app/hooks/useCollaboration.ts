...
interface CollaborationEvent {
  type: 'element_created' | 'element_updated' | 'element_deleted' | 'vote_added' | 'vote_removed';
  data: any;
  userId: string;
  timestamp: number;
}
...
export const useCollaboration = ({
  ...
  useEffect(() => {
    ...
    console.log('Initializing socket connection with token:', session.accessToken?.substring(0, 20) + '...');
    ...
    socket.on('user_joined', (user: { userId: string; name: string; avatar: string }) => {
      console.log('User joined:', user);
      setProjectUsers(prev => [...prev.filter(u => u.userId !== user.userId), {
        id: user.userId,
        userId: user.userId,
        name: user.name,
        avatar: user.avatar
  ...
  // Emit element events
  const emitElementCreated = useCallback((element: any) => {
    if (socketRef.current && isConnected) {
      socketRef.current.emit('element_created', element);
    }
  }, [isConnected]);

  const emitElementUpdated = useCallback((element: any) => {
    if (socketRef.current && isConnected) {
  ...
  const emitVoteAdded = useCallback((elementId: string, vote: any) => {
    if (socketRef.current && isConnected) {
      socketRef.current.emit('vote_added', { elementId, vote });
    }
  }, [isConnected]);

  const emitVoteRemoved = useCallback((elementId: string) => {
    if (socketRef.current && isConnected) {
