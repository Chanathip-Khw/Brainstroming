...
export const useElementResizing = ({
  ...
  const handleMouseMoveForResizing = useCallback((e: React.MouseEvent, isPanning: boolean, isDragging: boolean) => {
    if (isResizing && selectedElement && resizeHandle && !isPanning && !isDragging) {
      const rect = canvasRef.current?.getBoundingClientRect();
      if (rect) {
  ...
  }, [isResizing, selectedElement, resizeHandle, canvasRef, panX, panY, scale, resizeStart, elements, setElements]);
  ...
  const getResizeHandles = useCallback((element: CanvasElement) => {
    ...
    return handles.map(handle => ({
      ...handle,
      onMouseDown: (e: React.MouseEvent) => handleResizeMouseDown(element.id, handle.id, e, 'select')
    }));
